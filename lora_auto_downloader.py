...
class LoRAAutoDownloader:
    ...
    def process_workflow(self, workflow_data, civitai_token="", auto_download=True,
        ...
        status_lines.append(
            f"Found {len(loras_found)} LoRA references in workflow")
        ...
        for lora_info in loras_found:
            ...
            if exists:
                ...
                if auto_download and not check_only and civitai_token:
                    status_lines.append(
                        f"  Searching CivitAI for {lora_name}...")

                    # Search for the model
                    model_info = self.search_civitai_model(
                        lora_name, civitai_token)

                    if model_info:
                        status_lines.append(
                            f"  Found model: {model_info.get('name', 'Unknown')}")

                        # Download the model
                        if self.download_civitai_model(model_info, civitai_token):
                            status_lines.append(
                                f"  ✓ Successfully downloaded {lora_name}")
                        else:
                            status_lines.append(
                                f"  ✗ Failed to download {lora_name}")
                    else:
                        status_lines.append(
                            f"  ✗ Could not find {lora_name} on CivitAI")
                elif not civitai_token and auto_download:
                    status_lines.append(
                        f"  ⚠ CivitAI token required for download")
        ...
        if total_missing == 0:
            ...
            if not auto_download:
                status_lines.append(
                    "Set auto_download=True to download missing LoRAs")
