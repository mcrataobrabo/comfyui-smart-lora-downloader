...
class AutoLoRADetector:
    ...
    RETURN_NAMES = ("status_report", "missing_loras_list",
                    "downloaded_loras_list", "passthrough")
    ...
    def search_and_download_lora(self, lora_name, civitai_token):
        ...
        try:
            ...
            if HAS_NAME_MATCHER:
                ...
                print(
                    f"Enhanced matching found: '{best_match.get('name')}' (similarity: {similarity:.2f})")
                ...
                if not best_match:
                    ...
                    print(
                        f"No exact match found for '{lora_name}', using: '{best_match.get('name')}'")
    ...
    def check_lora_exists(self, lora_name):
        ...
        base_name = lora_name.replace(".safetensors", "").replace(
            ".ckpt", "").replace(".pt", "")
    ...
    def detect_and_handle_loras(self, civitai_token="", auto_download=True, check_missing=True, trigger_input=None):
        ...
        if check_missing:
            ...
            status_lines.append(
                "To use: Connect this node and run a workflow with missing LoRAs.")
            ...
            if len(lora_files) == 0:
                ...
                if len(lora_files) > 5:
                    status_lines.append(
                        f"  ... and {len(lora_files) - 5} more files")
        ...
        if missing_loras:
            ...
            if not auto_download:
                status_lines.append(
                    "Enable auto_download to download missing LoRAs")
            elif not civitai_token:
                status_lines.append(
                    "Provide CivitAI token to enable downloads")
        else:
            status_lines.append("\\n✅ LoRA directory ready!")
            status_lines.append(
                "💡 This node detects missing LoRAs from workflow validation errors.")
            status_lines.append(
                "💡 Run a workflow with missing LoRAs to trigger automatic detection.")

        return (
            "\\n".join(status_lines),
            json.dumps(missing_loras, indent=2),
            # New output for downloaded LoRAs
    ...
    def process_workflow_error(self, error_message, civitai_token="", auto_download=True):
        ...
        status_lines.append(
            f"Detected {len(missing_loras)} missing LoRA(s): {', '.join(missing_loras)}")

        for lora_name in missing_loras:
            if auto_download and civitai_token:
                status_lines.append(f"\\nAttempting to download: {lora_name}")
                success, message = self.search_and_download_lora(
                    lora_name, civitai_token)
        ...
        if downloaded_loras:
            status_lines.append(
                f"\\n✅ Successfully downloaded {len(downloaded_loras)} LoRA(s)")
            status_lines.append(
                "🔄 Please restart your workflow to use the new LoRAs")
