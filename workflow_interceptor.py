...
class WorkflowLoRAInterceptor:
            ...
            for lora_info in loras_found:
                ...
                if not exists:
                    ...
                    if self.civitai_token:
                        # Try to download
                        model_info = downloader.search_civitai_model(
                            lora_name, self.civitai_token)
                        if model_info:
                            if downloader.download_civitai_model(model_info, self.civitai_token):
                                downloaded_loras.append(lora_name)
                                logging.info(
                                    f"Auto-downloaded LoRA: {lora_name}")
    ...
    def configure_interceptor(self, enable_auto_mode, civitai_token, download_missing,
        ...
        if enable_auto_mode:
            status_lines.append(
                "✓ Auto-mode enabled - will scan all future workflows")
            if civitai_token:
                status_lines.append(
                    "✓ CivitAI token provided - will download missing LoRAs")
            else:
                status_lines.append(
                    "⚠ No CivitAI token - will only detect missing LoRAs")
        else:
            status_lines.append("○ Auto-mode disabled")
        ...
        if action == "list_loras":
            ...
            if not os.path.exists(lora_path):
                ...
                try:
                    ...
                    report_lines.append(
                        f"💾 Free space: {free / (1024**3):.1f} GB")
                except:
