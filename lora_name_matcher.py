...
class LoRANameMatcher:
    ...
    def __init__(self):
        self.skip_words = {'lora', 'model', 'sd', 'xl', 'v1', 'v2', 'the', 'and', 'or', 'of'}
    def normalize_name_for_search(self, name: str) -> str:
        ...
        name = re.sub(r'([a-z])(\d)', r'\1 \2', name)  # letter followed by number
        name = re.sub(r'(\d)([a-z])', r'\1 \2', name)  # number followed by letter
        
        # Convert separators to spaces
        name = re.sub(r'[_-]+', ' ', name)  # underscores and dashes
        
        # Remove version patterns
        name = re.sub(r'\bv\d+(\.\d+)?\b', '', name, flags=re.IGNORECASE)  # v1, v16, v1.5
        name = re.sub(r'\b(sd|xl)\d*(\.\d+)?\b', '', name, flags=re.IGNORECASE)  # SD1.5, XL
        ...
        for item in unique_results:
            ...
            for version in model_versions:
                ...
                for file_info in files:
                    ...
                    similarity = self.calculate_name_similarity(lora_name, file_name)
                    if similarity > best_similarity:
                        best_similarity = similarity
                        best_match = item
        ...
        for strategy_name, search_query in search_strategies:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    used_queries.append(f"{strategy_name}: '{search_query}' ({len(items)} results)")
                else:
                    used_queries.append(f"{strategy_name}: '{search_query}' (HTTP {response.status_code})")
            except Exception as e:
                used_queries.append(f"{strategy_name}: '{search_query}' (Error: {str(e)})")
